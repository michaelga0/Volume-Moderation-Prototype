...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption(option =>
      option
        .setName('threshold')
        .setDescription('Timeout threshold (must be an integer).')
        .setRequired(false)
    )
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (newThreshold === null) {
        await interaction.reply({
          content: `Current timeout threshold: **${serverSettings.timeout_threshold}**\nUsage: /settimeoutthreshold threshold:<integer>`,
          flags: MessageFlags.Ephemeral
        })
        return
      }
      ...
      // Update and save
      serverSettings.timeout_threshold = newThreshold
      await serverSettings.save()

      await interaction.reply({
        content: `Timeout threshold set to **${newThreshold}**.`,
        flags: MessageFlags.Ephemeral
      })
      writeLog(`Timeout threshold set to ${newThreshold} for guild ${guildId} by ${interaction.user.tag}`)

    } catch (error) {
      writeLog(`Failed to set timeout threshold for guild ${guildId}: ${error}`)
      await interaction.reply({
